...
function ModalIconButton({ children, className = "", label, onClick }) {
  return (
    <button
      ...
      aria-label={label}
      className={[
        "grid place-items-center rounded-full border border-white/40 text-white transition-[border-color,background,transform] duration-150 hover:scale-105 hover:border-white/70 hover:bg-white/10",
        className,
      ].join(" ")}
      onClick={onClick}
    >
