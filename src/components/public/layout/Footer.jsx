...
function Footer() {
  ...
  return (
    <footer className="grid mt-5 min-h-[284px] grid-cols-[auto_auto_auto] items-center justify-between border-t border-[rgba(231,227,252,0.23)] bg-[#181a1c] px-20 py-[60px] text-[rgba(193,194,196,0.88)] max-[900px]:min-h-[194px] max-[900px]:grid-cols-1 max-[900px]:gap-[18px] max-[900px]:px-5 max-[900px]:pb-[30px] max-[900px]:pt-[26px]">
      <div className="flex flex-col gap-[26px] max-[900px]:gap-3">
        <Link
          className="inline-flex items-center text-white max-[900px]:w-fit"
          to="/"
          aria-label="Chill home"
        >
          <img
            className="h-auto w-[163px] max-[900px]:w-[84px]"
            src={logo}
            alt="Chill"
          />
        </Link>
        <p className="m-0 text-base max-[900px]:text-xs">
      ...
      <nav className="block" aria-label="Genre">
        ...
        <div
          id="footer-genre-links"
          className={getLinksClassName("genre", true)}
        >
          {genreLinks.map((link) => (
            <Link
              key={link}
              className="transition-colors duration-[160ms] hover:text-white"
              to="/"
            >
              {link}
            </Link>
