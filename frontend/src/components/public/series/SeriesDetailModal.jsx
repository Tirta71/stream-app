...
function EpisodeRow({ episode, isActive }) {
  return (
    <article
      className={[
        "grid min-h-[92px] grid-cols-[36px_150px_minmax(0,1fr)_56px] items-center gap-5 rounded px-4 py-3 text-white transition-colors max-[640px]:min-h-[58px] max-[640px]:grid-cols-[18px_70px_minmax(0,1fr)_38px] max-[640px]:gap-2 max-[640px]:px-2 max-[640px]:py-2",
        isActive ? "bg-[#454545]" : "bg-transparent",
      ].join(" ")}
    >
      <span className="text-center text-sm font-medium text-white/86 max-[640px]:text-[10px]">
        {episode.number}
      </span>
      <img
        className="h-[64px] w-[150px] rounded-sm object-cover max-[640px]:h-[39px] max-[640px]:w-[70px]"
      ...
      <div className="min-w-0">
        <h3 className="m-0 truncate text-base font-bold leading-[1.25] max-[640px]:text-[11px]">
          {episode.title}
        </h3>
        <p className="m-0 mt-2 line-clamp-2 text-sm leading-[1.35] text-[#c1c2c4] max-[640px]:mt-1 max-[640px]:text-[9px]">
          {episode.description}
        </p>
      </div>
      <span className="self-start pt-1 text-right text-sm font-medium text-white/80 max-[640px]:text-[9px]">
        {episode.duration}
      </span>
    </article>
  );
...
function SeriesDetailModal({ detail, onClose }) {
  ...
  return (
    <motion.div
      animate="animate"
      className="fixed inset-0 z-[80] overflow-y-auto bg-[#080b0d]/80 px-6 py-14 text-white backdrop-blur-[1px] max-[640px]:px-5 max-[640px]:py-[76px]"
      initial="initial"
      role="presentation"
    ...
    >
      ...
      >
        <button
          type="button"
          aria-label="Tutup detail series"
        ...
        <section className="relative h-[430px] overflow-hidden bg-[#202124] max-[640px]:h-[210px]">
          <img
            ...
            onError={replaceBrokenImage}
          />
          <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(15,17,25,0.18)_0%,rgba(15,17,25,0.08)_56%,rgba(15,17,25,0.34)_100%),linear-gradient(180deg,rgba(24,26,28,0)_34%,rgba(24,26,28,0.6)_70%,#181a1c_100%)]" />
        ...
        <div className="px-[72px] pb-[72px] pt-10 max-[640px]:px-7 max-[640px]:pb-7 max-[640px]:pt-7">
          <div className="grid grid-cols-[minmax(0,1fr)_minmax(270px,0.9fr)] gap-12 max-[640px]:grid-cols-1 max-[640px]:gap-5">
            <div>
              <div className="flex flex-wrap items-center gap-3 text-sm font-semibold text-[#c1c2c4] max-[640px]:gap-2 max-[640px]:text-[10px]">
                ...
                <span className="inline-flex h-7 min-w-7 items-center justify-center rounded-full border border-white/45 px-2 text-xs text-white/86 max-[640px]:h-5 max-[640px]:min-w-5 max-[640px]:px-1.5 max-[640px]:text-[8px]">
                  {detail.ageRating}
                </span>
              </div>
          ...
          <section className="mt-14 max-[640px]:mt-7" aria-labelledby="episode-title">
            <h3
              id="episode-title"
